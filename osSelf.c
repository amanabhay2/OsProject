...
int main()
{
	...
	char swap2;
	//standard variables
	int turnAroundTime2=0,waitingTime2=0;
	...
	while(i<n)
	{
		printf("\n\tPlease enter Arrival time %d process\t",i+1);
		processName[i]=c;
	...
	for(;burstTimeDone<totalBurstTime;)
	{
		...
		if(remainingBurstTime[highPriority]==0)
		{
			...
			waitingTime[highPriority]=turnAroundTime[highPriority]-burstTime[highPriority];
	...
	averageWaitingTime=(float)waitingTime2/(float)n2;
	averageTurnAroundTime=(float)turnAroundTime2/(float)n2;
	printf("\n\n\t Average Waiting Time is : %f",averageWaitingTime);
	printf("\n\n\t Average Turn Around Time is : %f",averageTurnAroundTime);
	return 0;
}
